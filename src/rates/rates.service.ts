...
export class RatesService {
  ...
  getRates(baseRate: string, value: number): Observable<ConvertedRateDto[]> {
    return this.httpClient
      .get(this.url + '?apikey=' + this.apiKey + '&base_currency=' + baseRate)
      .pipe(
        map((axiosResponse: AxiosResponse) => {
          const dto = axiosResponse.data as BaseRateDto;
          const rates: ConvertedRateDto[] = [];
          for (const [rate, rateValue] of Object.entries(dto.data))
            rates.push(
              new ConvertedRateDto(
                rate,
                rateValue as number,
                (rateValue as number) * value,
              ),
            );
          return rates;
        }),
  ...
  getRateNames(): Observable<string[]> {
    return this.httpClient.get(this.url + '?apikey=' + this.apiKey).pipe(
      map((axiosResponse: AxiosResponse) => {
        const dto = axiosResponse.data as BaseRateDto;
        const rates: string[] = [];
        for (const [rate, rateValue] of Object.entries(dto.data))
